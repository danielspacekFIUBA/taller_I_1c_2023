...
impl Ahorcado {
    pub fn new(palabra: String) -> Self {
        ...
        let mut ahorcado = Ahorcado {
            palabra,
            letras_intentadas,
            letras_adivinadas,
            letras_equivocadas,
            palabra_oculta_actual,
            resuelto,
        };
    ...
    pub fn agregar_letra(&mut self, letra: String) -> bool {
        ...
        if self.letras_intentadas.contains(caracter) {
            return false;
        }
    ...
    fn completar_palabra(&mut self) {
        ...
        let resultado_incognita: Vec<String> = letras_palabra
            .iter()
            .map(|&x| {
